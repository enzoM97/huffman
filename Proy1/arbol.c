...
int comparador(void* val1, void* val2) {

	Values* a = (Values*)val1;
	Values* b = (Values*)val2;
	if (a->frecuencia < b->frecuencia) return -1;
	else if (a->frecuencia > b->frecuencia) return 1;
	else return 0;
}

void arbol_agregar(Arbol raiz, void* valor, int (*comparador)(void*,void*)) {
    _Arbol T = (_Arbol) raiz;
	Values* val = (Values*)arbol_valor(T);
    CONFIRM_RETURN(val);

    if (comparador(valor, val) < 0) {
        if (T->izq) {
